...
matrices_data* matrices_data::handle_ = nullptr;

const matrices_data & matrices_data::instance()
{
	if (handle_ == nullptr)
		handle_ = new matrices_data();
	return *handle_;
}

const vector_t matrices_data::x(size_t Np) const
{
...
const matrix_t matrices_data::m(size_t Np) const
{
	if (Np < MINNP || Np > MAXNP)
		throw(std::out_of_range("Np = " + xlib::i2s(Np)));
...
const matrix_t matrices_data::s(size_t Np) const
{
	if (Np < MINNP || Np > MAXNP)
		throw(std::out_of_range("Np = " + xlib::i2s(Np)));
...
void matrices_data::read_MAT_files()
{
	for (size_t i = MINNP; i <= MAXNP; i++)
	{
...
matrices_base::matrices_base(num_t h, size_t np)
	: h_(h), np_(np), is_init_(false), dim_(dimension())
{
}

void matrices_base::reset(num_t h, size_t np)
{
	h_ = h;
	np_ = np;
	initialize();
...
void matrices_base::memory_alloc()
{
...
void matrices1d::calculate()
{
	...
	lift_matrices_.at(static_cast<size_t>(InterfaceDirection::B))(1, 1) = 1.0;
	lift_matrices_.at(static_cast<size_t>(InterfaceDirection::F))(np_, np_) = 1.0;

}

void matrices1d::memory_alloc()
{
...
void matrices2d::calculate()
{
	...
	{
		InterfaceDirection idir = InterfaceDirection_list.at(i);
...
/*
system_matrix_angle::system_matrix_angle(num_t h, size_t np, num_t mu, num_t xi, num_t eta, num_t sigma)
	: matrices_(h, np), surface_matrices_(h, np), h_(h), np_(np), mu_(mu), xi_(xi), eta_(eta), sigma_(sigma)
{
	...
	{
		InterfaceDirection dir = InterfaceDirection_list.at(i);
		...
		if (dir == InterfaceDirection::B || dir == InterfaceDirection::F) {
			lift_matrix_.at(i).imatcopy('N', mu_);
			flux_matrix_.at(i).imatcopy('N', 1.0);
		}
		if (dir == InterfaceDirection::L || dir == InterfaceDirection::R) {
			lift_matrix_.at(i).imatcopy('N', xi_);
			flux_matrix_.at(i).imatcopy('N', 1.0);
		}
		if (dir == InterfaceDirection::D || dir == InterfaceDirection::U) {
	...
	if (mu_ >= 0)
	{
		InterfaceDirection dir = InterfaceDirection::B;
	...
	{
		InterfaceDirection dir = InterfaceDirection::F;
		idir = static_cast<size_t>(dir);
		tmpm.gemm(lift_matrix_.at(idir), matrices_.flux_matrix(dir));
		sysm_.omatadd(tmpm, sysm_, 1.0, 1.0);
	}
	...
	if (xi_ >= 0)
	{
		InterfaceDirection dir = InterfaceDirection::L;
		idir = static_cast<size_t>(dir);
		tmpm.gemm(lift_matrix_.at(idir), matrices_.flux_matrix(dir));
		sysm_.omatadd(tmpm, sysm_, 1.0, 1.0);
	}
	...
	{
		InterfaceDirection dir = InterfaceDirection::R;
		idir = static_cast<size_t>(dir);
		tmpm.gemm(lift_matrix_.at(idir), matrices_.flux_matrix(dir));
		sysm_.omatadd(tmpm, sysm_, 1.0, 1.0);
	}
	...
	if (eta_ >= 0)
	{
		InterfaceDirection dir = InterfaceDirection::D;
		idir = static_cast<size_t>(dir);
		tmpm.gemm(lift_matrix_.at(idir), matrices_.flux_matrix(dir));
		sysm_.omatadd(tmpm, sysm_, 1.0, 1.0);
	}
	...
	{
		InterfaceDirection dir = InterfaceDirection::U;
		idir = static_cast<size_t>(dir);
		tmpm.gemm(lift_matrix_.at(idir), matrices_.flux_matrix(dir));
		sysm_.omatadd(tmpm, sysm_, 1.0, 1.0);
	}
...
system_matrix_angle::~system_matrix_angle()
{
	if (ipiv != nullptr)
		delete[] ipiv;
	if (sysmp_ != nullptr)
...
std::ostream & operator<<(std::ostream & os, const system_matrix_angle & s)
{
	...
	{
		os << "direction " << InterfaceDirection_name_list.at(i).c_str() << ":" << std::endl;
		os << s.lift_matrix(InterfaceDirection_list.at(i));
	}	
	os << "==========================================" << std::endl;
	...
	{
		os << "direction " << InterfaceDirection_name_list.at(i).c_str() << ":" << std::endl;
		os << s.flux_matrix(InterfaceDirection_list.at(i));
	}
	os << "==========================================" << std::endl;
